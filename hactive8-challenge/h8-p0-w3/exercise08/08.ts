function pasanganTerbesar(num : number) {
    var numString : string= String(num)
    var pasanganTerbesar : number = Number(numString[0] + numString[1])
    
    for (var x = 0; x < numString.length - 1; x++) {
      
      var pasanganAngka : number = Number(numString[x] + numString[x + 1])
